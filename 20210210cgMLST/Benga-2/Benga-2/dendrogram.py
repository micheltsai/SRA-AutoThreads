...
class HierarchicalCluster:
    cluster_methods = {
        'single': fastcluster.single,
        'average': fastcluster.average,
    }

    def __init__(self, distmatrix, method='single'):
        if method not in self.cluster_methods.keys():
            raise "Method %s not support" % method
        self.method = method
        self.cluster = self.cluster_methods[method](squareform(distmatrix))
...
class Dendrogram:
    node_text_formatter = {
        'single': lambda x: f"{x:.0f}",
        'average': lambda x: f"{x:.2f}",
    }

    def __init__(self, profile, cluster_mehtod):
        distmatrix = PairwiseDistanceMatrix(profile)()
        self.hc = HierarchicalCluster(distmatrix, cluster_mehtod)
    ...
    def __call__(self, no_labels=False, show_node_info=False, labels_color=None, xlim=None):
        ...
        if show_node_info:
            ...
            for i, d in zip(icoord, dcoord):
                ...
                y = d[1]
                if y <= xmax:
                    text = self.node_text_formatter[self.hc.method](y)
...
def main():
    ...
    parser.add_argument("-m", "--method",
                        ...
                        default='single',
                        help="Cluster method, default is single.")
    ...
    dendrogram = Dendrogram(profile, cluster_mehtod=args.method)
