...
def prefetch_srav2(sralist,outdir):
    ...
    sra_dir=os.path.join(outdir,sralist)
    sra_file = os.path.join(outdir,"{}.sra".format(sralist))
...
def trimmingv2(forward_reads, reverse_reads, outdir, threads):
    ...
    cmd = f"/home/linsslab01/fastp -i {forward_reads} -I {reverse_reads} -o {paired_1} -O {paired_2} --length_required 36 --detect_adapter_for_pe --cut_front 3 --cut_tail 3 -w {threads} -j /dev/null -h /dev/null"
    run_cmd2(cmd)
    #progress_bar("trimming")
    print("#################### trimming END ####################\n")
    return paired_1, paired_2
...
def run_for_114v2(sra_id,sra_dir,fastq_dir,assemble_dir,outdir,threads,gsize,start,check_log):
    ...
    cmd = f"shovill --R1 {r1} --R2 {r2} --outdir {assemble_dir_} --depth 80 --tmpdir . --cpus {threads} --ram 8 --force"
    if gsize:
        cmd += f" --gsize {gsize}"
    print(cmd)
    run_cmd(cmd)
